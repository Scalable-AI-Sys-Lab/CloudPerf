...
ycsb_dirs = [
    ...
    'memcached_ycsb_a', 'memcached_ycsb_b', 'memcached_ycsb_c', 'memcached_ycsb_d', 'memcached_ycsb_f','memcached_uniform_a',
    'FASTER_A', 'FASTER_B', 'FASTER_C', 'FASTER_F', 'FASTER_UNIFORM_A', 'FASTER_UNIFORM_B', 'FASTER_UNIFORM_C', 'FASTER_UNIFORM_F'
]

# Output file name
output_file = 'contention_kv.xlsx'


# Number of latest entries to store
NUM_LAST_ENTRIES = 3  # Default value, can be changed dynamically
    ...
    if os.path.exists(result_file_path):
        ...
        if 'FASTER' in ycsb_dir:
            ...
            if len(timestamp_matches) >= NUM_LAST_ENTRIES and len(setup_time_matches) >= NUM_LAST_ENTRIES and len(throughput_matches) >= NUM_LAST_ENTRIES:
                ...
                last_timestamps = timestamp_matches[-NUM_LAST_ENTRIES:]
                last_setup_times = setup_time_matches[-NUM_LAST_ENTRIES:]
                last_throughputs = throughput_matches[-NUM_LAST_ENTRIES:]
                ...
                app_dataframes[ycsb_dir] = pd.DataFrame({
                    ...
                    'Throughput (ops/sec/thread)': last_throughputs
                })
            else:
            ...
            all_timestamps = timestamp_pattern.findall(content)
            ...
            if len(runtime_matches) >= NUM_LAST_ENTRIES and len(throughput_matches) >= NUM_LAST_ENTRIES and len(all_timestamps) >= NUM_LAST_ENTRIES:
                # Get the last NUM_LAST_ENTRIES entries
                last_runtimes = runtime_matches[-NUM_LAST_ENTRIES:]
                last_throughputs = throughput_matches[-NUM_LAST_ENTRIES:]
                last_timestamps = all_timestamps[-NUM_LAST_ENTRIES:]
