...
# cd YCSB
sed -i 's/recordcount=.*/recordcount=5000000/' $YCSB_DIR/workloads/workloade
sed -i 's/operationcount=.*/operationcount=5000000/' $YCSB_DIR/workloads/workloade
sed -i 's/requestdistribution=.*/requestdistribution=zipfian/' $YCSB_DIR/workloads/workloade
...
cd $YCSB_DIR
# insert data
python3 ./bin/ycsb load memcached -s -P ./workloads/workloade -p "memcached.hosts=127.0.0.1" -p "threadcount=2"
