...
# Patterns for detecting the start, stop of a run, timestamp, and latency values
start_pattern = re.compile(r"Running\s+([\w.]+)\s+\(Run\s+(\d+)\)")
stop_pattern = re.compile(r"Stopped\s+([\w.]+)\s+\(Run\s+(\d+)\)")
...
with open(log_file_path, 'r') as log_file:
    ...
    for line in log_file:
        ...
        if stop_match and current_benchmark == stop_match.group(1):
            # When a run stops, save the data for the current run
            if current_benchmark and latency_buffer:
                if current_benchmark not in benchmark_data:
                    benchmark_data[current_benchmark] = {"Run": [], "Start Time": [], "Latencies": [], "Timestamps": []}
            ...
            # Clear the current benchmark and run data
            current_benchmark = None
            current_run = None
            latency_buffer.clear()
            timestamp_buffer.clear()
            print(f"Run stopped: {stop_match.group(1)} (Run {stop_match.group(2)})")
    ...
    # After the loop, ensure the last run's data is saved
    if current_benchmark and latency_buffer:
        if current_benchmark not in benchmark_data:
            benchmark_data[current_benchmark] = {"Run": [], "Start Time": [], "Latencies": [], "Timestamps": []}
...
save_file_name = "contention_spark_vector"
...
print(f"Data has been successfully written to '{save_file_name}.xlsx'")
