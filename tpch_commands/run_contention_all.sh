...
local_memory_limit=$((20*1024*1024*1024))
echo "local memory limit is: $local_memory_limit"
sudo ../general_commands/update_mem_limit -cg_idx 1 -mem_limit_value $local_memory_limit
...
for i in {2..22}; do
  ...
  if [ -f "./run_exp.sh" ]; then
    ...
    sudo ../../general_commands/log_memory_usage -cg_idx 1 -output_dir "$(pwd)" -output_file "memory_usage_log_vector.csv" &
    # sleep 5
