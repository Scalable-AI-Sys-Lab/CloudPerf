...
export RESULT_DIR=../../results/tpch/q12
...
./track_pid_mercury.sh "postgres" 1 &
popd > /dev/null
...
sudo bash -c "cat $PG_TPCH_DIR/perfdata-10GB/q12/exectime.txt >> $RESULT_DIR/result_app_perf.txt"
echo "Execution time appended to $RESULT_DIR/result_app_perf.txt"

sudo pkill -f "log_memory_usage"
sudo pkill -f "track_pid_mercury.sh"