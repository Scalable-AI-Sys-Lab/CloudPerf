...
if ! check_server; then
  # Start the server
  numactl --physcpubind=49-52,105-108 python /data/Mercury_my/apps/vectorDB/vector-server.py &
fi
...
for ((i = 0; i < num_instances; i++)); do
  numactl --physcpubind=49-52,105-108 python /data/Mercury_my/apps/vectorDB/client.py &
done
