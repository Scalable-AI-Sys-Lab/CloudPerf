...
dirs = ['602.gcc_s','603.bwaves_s','605.mcf_s','607.cactuBSSN_s','619.lbm_s','631.deepsjeng_s','638.imagick_s','649.fotonik3d_s','654.roms_s','657.xz_s']
    ...
    if os.path.exists(file_path):
        with open(file_path, 'r') as file:
            ...
            if timestamps and len(blocks) > 1:
                # Get the latest block and its corresponding timestamp
                latest_timestamp = timestamps[-1]
                latest_block = blocks[-1].strip()
                
                # Extract real, user, and sys times from the latest block
                real = re.search(r'real (\d+\.\d+)', latest_block)
                user = re.search(r'user (\d+\.\d+)', latest_block)
                sys = re.search(r'sys (\d+\.\d+)', latest_block)
                ...
                data[full_name] = pd.DataFrame({
                    ...
                    'User': [float(user.group(1)) if user else None],
                    'Sys': [float(sys.group(1)) if sys else None]
                })
            else:
...
output_file = 'run_single_summary.xlsx'
